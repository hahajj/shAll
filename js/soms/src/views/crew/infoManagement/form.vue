...
export default {
  ...
  data() {
    return {
      parent: '',
      options: [{ value: '在船', label: '在船' }, { value: '下船', label: '下船' }],
      value: '',
      partyTypes: '',
      jobTypes: [],
  ...
  methods: {
    queryAllOrganization() {
      const para = { page: 0, size: 9999, organizationType: 'SHIP_DEPT' }
    ...
    getAllJob() {
      const para = { page: 0, size: 9999, jobType: 'SHIP' }
      getAllJob(para).then(res => {
