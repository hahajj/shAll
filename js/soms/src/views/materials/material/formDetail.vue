<template>
  <el-dialog :append-to-body="true" :close-on-click-modal="false" :visible.sync="deltailDialog" :before-close="resetForm" :title="isDetail?(isDetail === '到船签收' ? '到船签收' : '签收详情'):'供货信息'" width="600px">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button v-if="isDetail === '到船签收' || supplyDialog" type="primary" @click="doSubmit">确认</el-button>
    </div>
  </el-dialog>
</template>

<script>
import { addArrived, delFile, contentPreview, getArrived, getArrivedId, addSupplyInfo, editSupplyInfo } from '@/api/material'
...
export default {
  ...
  methods: {
    ...
    getArrivedId(data) {
      const idArr = []
      this.loading = true
      getArrivedId({ materialPurchaseId: data.materialPurchaseId }).then(res => {
        if (res.content.length > 0) {
          res.content.map(v => {
            idArr.push(v.materialPurchaseArrivedId)
          })
          if (idArr.length === 1) {
            this.materialPurchaseArrivedId = idArr[0]
          } else {
            this.materialPurchaseArrivedId = idArr.join(',')
          }
          getArrived(this.materialPurchaseArrivedId).then(res => {
            this.loading = false
            this.contents = res
          })
        } else {
