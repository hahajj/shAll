...
<script>
import { add, edit, get, delFile, getmaterialPart, contentPreview, getContentsById } from '@/api/material'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        // materialPartInventoryAmount: [{ required: true, message: '请输入库存数量', trigger: 'blur' }]
      }
    }
  },
  methods: {
    ...
    handleMaterialName(data) {
      ...
      if (Info.materialInventory) {
        this.form.materialPartInventoryAmount = Info.materialInventory.amount
      }
      if (Info.unitName) {
    ...
    // 获取某个物资的图片信息
    reloadForm() {
      getContentsById(this.materialPurchaseId).then(res => {
        this.contents = res
      })
    },
    cancelInnerVisible() {
