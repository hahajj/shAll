<template>
  ...
  >
    <el-form ref="form" :model="form" :rules="rules" size="small" label-width="120px">
      ...
      <el-form-item v-if="isAdd" label="接收方" prop="recevieList">
        <el-select v-model="form.recevieList" :disabled="!isAdd" multiple placeholder="请选择">
          <el-option
            v-for="item in shipOrganizations"
            :key="item.id"
            :label="item.label"
            :value="item.id"/>
        </el-select>
      ...
      <span v-if="!isAdd" style="color: rgb(24, 144, 255);font-size: 20px">接收方</span>
      <el-table v-loading="loading" v-if="!isAdd" :data="recevieList" size="small" style="width: 100%;margin: 10px 0 20px 0;">
        <el-table-column prop="organizationName" label="船舶名称" align="center"/>
        <el-table-column prop="createdDate" label="创建日期" align="center">
          <template slot-scope="scope">
            <span>{{ parseTime(scope.row.createdDate) }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="receiveDate" label="接收日期" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column prop="status" label="接收状态" align="center" >
          <template slot-scope="scope">
            <span v-if="scope.row.confirm==true" style="color: #13ce66;">已接收</span>
            <span v-else style="color: #F56C6C;">未接收</span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
    ...
    return {
      ...
      rules: {
        circularType: [
          ...
          { required: true, message: '请选择接收方', trigger: 'blur' }
        ], title: [
  ...
  methods: {
    ...
    doSubmit() {
      console.log(this.form.circularType)
      ...
      fd.append('recevieList', this.form.recevieList)
