    ...
    <el-tabs :before-leave="beforeLeave" v-model="activeName" type="card" @tab-click="handleClick">
      <el-tab-pane label="备件库存盘点" name="equipment">
        ...
        <el-table v-loading="loading" id="equipmentTable" ref="equipmentTable" :data="data" size="small" style="width: 100%;" highlight-current-row stripe @sort-change="sortChange">
          ...
          <el-table-column prop="equipmentPartSpec.equipmentPartId.name" label="备件名称" align="center"/>
          <el-table-column prop="equipmentPartSpecName" label="备件规格" align="center"/>
          <el-table-column prop="equipmentPartSpec.equipmentPartId.code" label="备件号" align="center"/>
          <el-table-column prop="equipmentPartSpec.unitName" label="单位" align="center"/>
          <el-table-column prop="arrivedAmount" label="本月新增数量" align="center"/>
          <el-table-column prop="consumeAmount" label="本月消耗数量" align="center"/>
          <el-table-column prop="lastMonthAmount" label="上月库存数量" align="center"/>
          <el-table-column prop="amount" label="当前库存数量" align="center"/>
          <el-table-column v-if="$store.state.user.user.partyDTO.partyType.partyType==='CREW' || $store.state.user.user.partyDTO.partyType.partyType==='SYSADMIN'" label="操作" width="150px" align="center">
            <template slot-scope="scope">
              <el-button v-permission="['ADMIN','S2LEQUIPMENTPARTPURCHASE_ALL','S2LEQUIPMENTPARTPURCHASE_EDIT']" size="mini" type="primary" icon="el-icon-edit" @click="edit(scope.row)"/>
              <el-button v-permission="['ADMIN','S2LEQUIPMENTPARTPURCHASE_ALL','S2LEQUIPMENTPARTPURCHASE_DELETE']" slot="reference" type="danger" icon="el-icon-delete" size="mini" @click="open(scope.row)"/>
            </template>
          </el-table-column>
        </el-table>
        <!--分页组件-->
      ...
      <el-tab-pane label="物资库存盘点" name="material">
        ...
        <el-table v-loading="materialLoading" id="materialTable" ref="materialTable" :data="materialData" size="small" style="width: 100%;" highlight-current-row stripe @sort-change="sortChange">
          <el-table-column type="index" label="序号" align="center"/>
          <el-table-column v-if="$store.state.user.user.partyDTO.partyType.partyType==='EMPLOYEE' || $store.state.user.user.partyDTO.partyType.partyType==='SYSADMIN'" prop="organizationId.name" label="船舶" align="center"/>
          <el-table-column prop="materialSpec.materialId.materialClassCode.materialClassName" label="物资大类" align="center"/>
          <el-table-column prop="materialSpec.materialId.name" label="物资名称" align="center"/>
          <el-table-column prop="materialSpec.name" label="物资规格" align="center"/>
          <el-table-column prop="materialSpec.unitName" label="单位" align="center"/>
          <el-table-column prop="arrivedAmount" label="本月新增数量" align="center"/>
          <el-table-column prop="consumeAmount" label="本月消耗数量" align="center"/>
          <el-table-column prop="lastMonthAmount" label="上月库存数量" align="center"/>
          <el-table-column prop="amount" label="当前库存数量" align="center"/>
          <el-table-column v-if="$store.state.user.user.partyDTO.partyType.partyType==='CREW' || $store.state.user.user.partyDTO.partyType.partyType==='SYSADMIN'" label="操作" width="150px" align="center">
            <template slot-scope="scope">
              <el-button v-permission="['ADMIN','S2LMATERIALPURCHASE_ALL','S2LMATERIALPURCHASE_EDIT']" size="mini" type="primary" icon="el-icon-edit" @click="edit(scope.row)"/>
              <el-button v-permission="['ADMIN','S2LMATERIALPURCHASE_ALL','S2LMATERIALPURCHASE_DELETE']" slot="reference" type="danger" icon="el-icon-delete" size="mini" @click="open(scope.row)"/>
            </template>
          </el-table-column>
        </el-table>
        <!--分页组件-->
...
export default {
  ...
  methods: {
    ...
    beforeInit() {
      this.url = 'api/s2lEquipmentPartInventory'
    ...
    open(data) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        if (this.tabData === '备件库存盘点') {
          this.subDelete(data.equipmentPartInventoryId)
        }
        if (this.tabData === '物资库存盘点') {
          this.subDelete(data.materialInventoryId)
        }
      }).catch(() => {})
    },
    subDelete(id) {
    ...
    edit(data) {
      ...
      if (this.tabData === '备件库存盘点') {
        _this.equipment = {
          ...
          name: data.equipmentPartSpecName,
          code: data.equipmentPartSpec.equipmentPartId.code,
          arrivedAmount: data.arrivedAmount,
          consumeAmount: data.consumeAmount,
          lastMonthAmount: data.lastMonthAmount,
          amount: data.amount,
          miniumAmount: data.miniumAmount,
          equipmentPartSpecId: data.equipmentPartSpec.equipmentPartSpecId,
          equipmentPartInventoryId: data.equipmentPartInventoryId,
          organizationId: this.$store.state.user.user.partyDTO.organization.organizationId
        }
        // _this.equipment.equipmentPartInventoryId = data.equipmentPartSpec.equipmentPartInventoryId
      ...
      if (this.tabData === '物资库存盘点') {
        ...
        _this.material = {
          checked: false,
          materialName: data.materialSpec.materialId.name + ' ' + data.materialSpec.name,
          materialSpec: data.materialSpec.materialId.name,
          materialSpecName: data.materialSpec.name,
          arrivedAmount: data.arrivedAmount,
          mConsumeAmount: data.consumeAmount,
          lastMonthAmount: data.lastMonthAmount,
          amount: data.amount
        }
        _this.newMaterial = {
