          ...
          <el-table v-loading="loading" :data="data" highlight-current-row size="small" style="width: 100%;" @current-change="handleCurrentChange">
            ...
            <el-table-column v-if="checkPermission(['ADMIN','ROLES_ALL','ROLES_EDIT','ROLES_DELETE'])" label="操作" width="130px" align="center">
              <template slot-scope="scope">
                ...
                <el-button v-permission="['ADMIN','ROLES_ALL','ROLES_DELETE']" slot="reference" type="danger" icon="el-icon-delete" size="mini" @click="open(scope.row)"/>
              </template>
            </el-table-column>
          </el-table>
          <!--分页组件-->
...
export default {
  ...
  methods: {
    ...
    edit(data) {
      ...
      _this.form = { id: data.id, name: data.name, remark: data.remark, organizations: data.organizations, dataScope: data.dataScope, level: data.level }
      for (let i = 0; i < _this.form.organizations.length; i++) {
        _this.organizationIds[i] = _this.form.organizations[i].id
      }
      _this.dialog = true
    }
  }
