      ...
      <el-col :xs="17" :sm="18" :md="20" :lg="20" :xl="20">
        ...
        <el-table v-loading="loading" :data="data" size="small" style="width: 100%;" @sort-change="sortChange">
          ...
          <el-table-column v-if="checkPermission(['ADMIN','PARTY_ALL','PARTY_EDIT','PARTY_DELETE'])" label="操作" width="250px" align="center">
            <template slot-scope="scope">
              <!-- 修改编辑按钮 -->
              <el-button v-permission="['ADMIN','PARTY_ALL','PARTY_EDIT']" size="mini" type="primary" icon="el-icon-edit" @click="edit(scope.row)"/>
              <div style="display: inline-block;margin: 0px 2px;">
              ...
              <!-- 删除按钮 -->
              <el-button v-permission="['ADMIN','PARTY_ALL','PARTY_DELETE']" slot="reference" type="danger" icon="el-icon-delete" size="mini" @click="open(scope.row)"/>
            </template>
          </el-table-column>
        </el-table>
        <!--分页组件-->
...
import { uploadFile, downloadFile } from '@/api/employ'
...
export default {
  ...
  methods: {
    ...
    beforeInit() {
      ...
      this.params['partyType'] = 'EMPLOYEE'
      this.params['statusSet'] = '试用,在职,劝退离职,主动离职,退休,实习'
      return true
    },
    ...
    getDeptDatas() {
      ...
      this.params = { sort: sort, page: 0, size: 9999, 'organizationType': 'PARENT_COMPANY,LAND_DEPT,BRANCH_COMPANY,DEPT,COMPANY' }
    ...
    download() {
      ...
      this.partyQueryCriteria.size = 9999
      this.partyQueryCriteria.partyType = 'EMPLOYEE'
      this.partyQueryCriteria.statusSet = ['在职', '实习', '试用', '劝退离职', '主动离职']
      downloadFile(this.partyQueryCriteria).then(res => {
        exceldata = res
        this.downloadLoading = true
        import('@/utils/export2Excel').then(excel => {
          const tHeader = ['序', '社保账号', '部门', '姓 名', '状 态', '职务', '身份证号码', '性别', '出生日期', '年龄', '生肖', '学历', '毕业学校', '专  业', '政治面貌', '职业资格', '联系电话',
            '电信号码', '籍贯', '户籍地址', '实际居住地址（详细：省、市、区/县）', '紧急联系人', '紧急联系电话', '银行卡号', '入司时间', '试用期', '转正时间', '首份合同开始', '最近合同开始',
            '合同到期日期', '合同到期提醒(天)', '工龄1', '工龄', '生日提醒', '备注']
          const filterVal = ['index', 'socialSecurityAccount', 'organization', 'name', 'status', 'job', 'idNumber', 'sex', 'birthday', 'age', 'animalZodiac', 'education', 'school', 'major', 'politicalStatus', 'vocationalQualification',
            'contactMech', 'companyGroupPhone', 'nativePlace', 'homeAddress', 'residence', 'emergencyContact', 'emergencyContactPhone', 'bankAccount', 'entryDate', 'probationMonth', 'conversionDate', 'firstContractStartDate', 'contractStartDate',
            'contractExpirationDate', 'contractRemind', 'serviceLength1', 'serviceLength', 'birthdayRemind', 'comments']
    ...
    downloadTemplate() {
      if (process.env.NODE_ENV === 'production') {
        ...
        if (url.includes('/ssms/')) {
          window.location.href = '/ssms/static/template/岸基人员信息属性.xlsx'
        } else if (url.includes('soms')) {
          window.location.href = '/soms/static/template/岸基人员信息属性.xlsx'
        } else {
          window.location.href = '/static/template/岸基人员信息属性.xlsx'
        }
      } else {
    ...
    uploadFile(params) {
      try {
        uploadFile(params.file, 'EMPLOYEE').then(() => {
          this.$notify({
            title: '上传成功',
            type: 'success',
            duration: 2500
